...
def generate_strided_v1_0(n_accesses=500000, stride=4, base=0, n_blocks_total=100000):
    """
    Strided / Regular Multi-Stride Access:
    Addresses jump by a constant stride S > 1.
    Address k = Base + (k * S) (mod Total Size)
    Simulates column-major / tensor slicing access skipping intermediate blocks.
    """
    blocks = (base + np.arange(n_accesses) * stride) % n_blocks_total
    ops = np.random.choice(['read', 'write'], size=n_accesses, p=[0.95, 0.05])
    ...
    return normalize_trace(df, "strided")

def generate_graph_walk_v1_0(n_accesses=500000, n_nodes=10000):
    """
    Pointer-Chasing / Irregular Graph Walks (GNNs, SpMV, B-Trees):
    Address(t+1) = Data[Address(t)]
    Traverses scale-free graph adjacency lists and fetches node feature vectors.
    """
    rng = np.random.default_rng(42)
    degrees = np.clip(rng.zipf(1.5, size=n_nodes), 2, 64)
    adj_table = [rng.integers(0, n_nodes, size=int(d)) for d in degrees]
    
    curr = 0
    blocks = np.empty(n_accesses, dtype=int)
    for i in range(n_accesses):
        blocks[i] = curr
        if rng.random() < 0.10:
            curr = int(rng.integers(0, n_nodes))
        else:
            curr = int(rng.choice(adj_table[curr]))
            
    ops = np.random.choice(['read', 'write'], size=n_accesses, p=[0.90, 0.10])
    df = pd.DataFrame({
        "block_id": blocks,
        "op": ops,
        "size_kb": 4,
    ...
    return normalize_trace(df, "graph_walk")
...
def generate_strided_trace_legacy(n_blocks_total=10000, n_accesses=50000, stride=4):
    """ Legacy small-scale strided synthetic for local testing """
    trace_blocks = (np.arange(n_accesses) * stride) % n_blocks_total
    df = pd.DataFrame([{"block_id": int(b), "phase": 3} for b in trace_blocks])
    return normalize_trace(df, "strided")

def generate_graph_walk_trace_legacy(n_blocks_total=10000, n_accesses=50000):
    """ Legacy small-scale graph walk synthetic for local testing """
    rng = np.random.default_rng(42)
    adj_table = [rng.integers(0, n_blocks_total, size=4) for _ in range(n_blocks_total)]
    curr = 0
    blocks = []
    for _ in range(n_accesses):
        blocks.append(curr)
        curr = int(rng.choice(adj_table[curr]))
    df = pd.DataFrame([{"block_id": int(b), "phase": 4} for b in blocks])
    return normalize_trace(df, "graph_walk")
...
if __name__ == "__main__":
    ...
    if args.mode == "real":
        ...
        df_strided = load_real_traces("./processed_traces/", "strided") if os.path.exists("./processed_traces/strided_normalized.parquet") else generate_strided_v1_0()
        df_graph = load_real_traces("./processed_traces/", "graph_walk") if os.path.exists("./processed_traces/graph_walk_normalized.parquet") else generate_graph_walk_v1_0()
    else:
        if args.scale == "real":
            ...
            df_strided = generate_strided_v1_0()
            df_graph = generate_graph_walk_v1_0()
            ...
            df_strided.to_parquet("./processed_traces/strided_normalized.parquet", engine='pyarrow')
            df_graph.to_parquet("./processed_traces/graph_walk_normalized.parquet", engine='pyarrow')
        else:
            print("Generating Synthetic Traces (Scale: LOCAL)...")
            df_resnet = generate_resnet_trace_legacy()
            df_bert = generate_bert_trace_legacy()
            df_strided = generate_strided_trace_legacy()
            df_graph = generate_graph_walk_trace_legacy()
            ...
            df_strided.to_csv("dlio_strided_trace.csv", index=False)
            df_mixed.to_csv("dlio_mixed_trace.csv", index=False)
    
    # Validation Assertion: check row count
    check_workloads = [("ResNet", df_resnet), ("BERT", df_bert), ("Strided", df_strided), ("GraphWalk", df_graph if 'df_graph' in locals() else df_resnet)]
    if 'df_unet' in locals():
        check_workloads.append(("UNet3D", df_unet))
    for name, df in check_workloads:
    ...
    print(f"Strided  : {len(df_strided)} rows")
    if 'df_graph' in locals():
        print(f"GraphWalk: {len(df_graph)} rows")
